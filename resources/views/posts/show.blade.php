 ...
 <div class="container mx-auto md:flex gap-2">
            ...
            @if ($post->user_id===auth()->user()->id)
                <form action="{{route('posts.destroy',$post)}}" method="POST">
                    ...
                    <input 
                        ...
                        class="bg-red-500 hover:bg-red-700 p-2 rounded text-white font-bold mt-4 
                        cursor-pointer"    
                    >
    ...
    <div class="md:w-1/2 p-5">
        <div class="shadow bg-white p-5 mb-5">
            ...
            @if (session('mensaje'))
                <div class="bg-green-700 text-white text-center uppercase font-bold p-2 rounded-lg mb-6">
                    {{session('mensaje')}}
                </div>
            @endif
            ...
            <form action="{{route('comentarios.store',['post' => $post,'user' => $user])}}" method="POST">
                ...
                <input 
                    type="submit" 
                    value="Comentar"
                    class="bg-sky-500 hover:bg-sky-700 transition-colors cursor-pointer uppercase font-bold w-full
