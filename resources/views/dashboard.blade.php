                ...
                @auth
                    @if ($user->id !== auth()->user()->id)
                        @if (!$user->siguiendo( auth()->user() ))
                            ...
                            @csrf
                            <input type="submit" class="bg-blue-500 text-white uppercase rounded-lg px-3 py-1
                        ...
                        @else
                            <form action="{{route('users.unfollow',$user)}}" method="POST">
                                @csrf
                                @method('DELETE')
                                <input type="submit" class="bg-red-500 text-white uppercase rounded-lg px-3 py-1
